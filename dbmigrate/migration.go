...
func Process(args []string) {
	if Config.Path == "" {
		fmt.Println("please set the path configuration")
		return
	}
	if len(args) == 0 {
...
func handleMake(args []string) {
	if len(args) < 2 {
		fmt.Println("Please provide name of the migration to create")
		return
	}
	newPath := path.Join(Config.Path, fmt.Sprintf("%s-%s.sql", time.Now().Format("20060102-150405"), args[1]))
	err := ioutil.WriteFile(newPath, []byte(`--dbmigrate:up
